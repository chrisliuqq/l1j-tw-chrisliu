...
public class L1DwarfInventory extends L1Inventory {
	...
	// ÇcÇaÇÃcharacter_warehouseÇ÷ìoò^
	@Override
	...
	// ÇcÇaÇÃcharacter_warehouseÇçXêV
	@Override
	...
	// ÇcÇaÇÃcharacter_warehouseÇ©ÇÁçÌèú
	@Override
	...
	private static void present(ArrayList<String> accountList, int itemid,
		...
		L1Item temp = ItemTable.getInstance().getTemplate(itemid);
		if (temp == null) {
			throw new Exception("ë∂ç›ÇµÇ»Ç¢ÉAÉCÉeÉÄID");
		}
		Connection con = null;
		PreparedStatement pstm = null;
		try {
			...
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
			throw new Exception(".presentèàóùíÜÇ…ÉGÉâÅ[Ç™î≠ê∂ÇµÇ‹ÇµÇΩÅB");
		} finally {
			SQLUtil.close(pstm);
			SQLUtil.close(con);
		}
