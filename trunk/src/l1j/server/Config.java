...
public final class Config {
	...
	// àÍâÒÇÃçUåÇÇ≈ãzÇ¶ÇÈMPå¿äE(SOMÅAç|ìSSOMÅj
	...
	public static void load() {
		...
		try {
			...
			ALT_GMSHOP_MIN_ID = Integer.parseInt(altSettings.getProperty(
					"GMshopMinID", "0xffffffff")); // éÊìæé∏îséûÇÕñ≥å¯
			ALT_GMSHOP_MAX_ID = Integer.parseInt(altSettings.getProperty(
					"GMshopMaxID", "0xffffffff")); // éÊìæé∏îséûÇÕñ≥å¯
	...
	private static void validate() {
		if (!IntRange.includes(Config.ALT_ITEM_DELETION_RANGE, 0, 5)) {
			throw new IllegalStateException("ItemDeletionRangeÇÃílÇ™ê›íËâ¬î\îÕàÕäOÇ≈Ç∑ÅB");
		}

		if (!IntRange.includes(Config.ALT_ITEM_DELETION_TIME, 1, 35791)) {
			throw new IllegalStateException("ItemDeletionTimeÇÃílÇ™ê›íËâ¬î\îÕàÕäOÇ≈Ç∑ÅB");
		}
	}
